...
/* task->mm, skb->sk, req->sk are not RCU protected, but we mark them as such
 * to allow non-sleepable bpf progs dereference them
 */
BTF_TYPE_SAFE_RCU_OR_NULL(struct task_struct) {
	struct mm_struct *mm;
};
...
static bool type_is_rcu_or_null(struct bpf_verifier_env *env,
				...
				const char *field_name, u32 btf_id)
{
	BTF_TYPE_EMIT(BTF_TYPE_SAFE_RCU_OR_NULL(struct task_struct));
