...
/* Benchmark for BPF_MAP_TYPE_QP_TRIE lookup */

#include <sys/random.h>
#include <argp.h>
#include "bench.h"
#include "bpf_qp_trie_lookup.skel.h"
#include "bpf_util.h"

/* BPF triggering benchmarks */
static struct ctx {
	struct bpf_qp_trie_lookup *skel;
} ctx;
...
static const struct argp_option opts[] = {
	{ "key_size", ARG_KEY_SIZE, "KEY_SIZE", 0,
	  "The qp-trie key size (max 1024)"},
	{ "max_entries", ARG_MAX_ENTRIES, "MAX_ENTRIES", 0,
	  "The qp-trie max entries"},
...
const struct argp bench_qp_trie_lookup_argp = {
	.options = opts,
	.parser = parse_arg,
};
...
static void setup(void)
{
	...
	ctx.skel = bpf_qp_trie_lookup__open();
	if (!ctx.skel) {
		fprintf(stderr, "failed to open skeleton\n");
		exit(1);
	}

	bpf_map__set_max_entries(ctx.skel->maps.qp_trie_bench, args.max_entries);
	bpf_map__set_key_size(ctx.skel->maps.qp_trie_bench, args.key_size);
	bpf_map__set_value_size(ctx.skel->maps.qp_trie_bench, 8);
	...
	ret = bpf_qp_trie_lookup__load(ctx.skel);
	if (ret) {
		bpf_qp_trie_lookup__destroy(ctx.skel);
		fprintf(stderr, "failed to load map: %s", strerror(-ret));
		exit(1);
	}

	/* fill in the qp_trie */
	map_fd = bpf_map__fd(ctx.skel->maps.qp_trie_bench);
...
const struct bench bench_bpf_qp_trie_lookup = {
	.name = "bpf-qp-trie-lookup",
	.argp = &bench_qp_trie_lookup_argp,
	.validate = validate,
	.setup = setup,
