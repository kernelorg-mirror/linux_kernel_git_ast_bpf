...
static __always_inline void tcp_estats_event_init(
	struct tcp_estats_event *event,
...
static __always_inline void tcp_estats_init(struct sock *sk,
	...
	enum tcp_estats_event_type type)
{
	tcp_estats_event_init(event, type);
	tcp_estats_conn_id_init(conn_id, sk);
}

static __always_inline void send_basic_event(struct pt_regs *ctx,
	struct sock *sk,
	enum tcp_estats_event_type type)
{
	...
	bpf_perf_event_output(ctx, &perf_event_map, bpf_get_smp_processor_id(),
		&ev, sizeof(ev));
}

SEC("tcp_send_active_reset")
int _tcp_send_active_reset(struct pt_regs *ctx)
{
	struct sock *sk;
	...
	send_basic_event(ctx, sk, TCP_ESTATS_TX_RESET);
	return 0;
}
