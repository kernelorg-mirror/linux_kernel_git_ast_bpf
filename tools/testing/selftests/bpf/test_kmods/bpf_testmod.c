			 ...
			 struct pt_regs *regs)
{
	...
	if (test_kmalloc && (rval & (1 << 9))) {
		for (i = 0; i < page_cnt; i++) {
			o = kmalloc_nolock(16, __GFP_ACCOUNT, -1);
			try_kmalloc_cnt++;
			if (!o) {
				try_kmalloc_fail++;
				break;
...
static int page_alloc_kthread(void *arg)
{
	...
	for (i = 0; i < ALLOC_CNT && test_kmalloc; i++) {
		...
		for (j = 0; j < obj_cnt; j++) {
			o = kmalloc(16, GFP_KERNEL_ACCOUNT);
			if (!o)
				break;
			objs[j] = o;
		}
...
static int page_alloc_test(void)
{
	...
	while (!wait_for_completion_timeout(&wait, msecs_to_jiffies(2000))) {
	...
	/*
	 * No need to obj_cgroup_put(objcg);
	 * We're still in the context of the task loading this module.
	 */
