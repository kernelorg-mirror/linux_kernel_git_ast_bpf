...
#include "arena_htab_llvm.skel.h"
...
static void test_arena_htab_llvm(void)
{
	...
	struct arena_htab_llvm *skel;
	struct htab *htab;
	size_t arena_sz;
	void *area;
	int ret;

	skel = arena_htab_llvm__open_and_load();
	if (!ASSERT_OK_PTR(skel, "arena_htab_llvm__open_and_load"))
		return;
	...
	*(volatile int*)area = 0x55aa;
	...
	if (skel->bss->skip) {
		printf("%s:SKIP:no __builtin_bpf_arena_cast\n", __func__);
		test__skip();
		goto out;
	}
